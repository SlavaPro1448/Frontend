...
def update_supabase_auth_status(phone, is_authenticated=True):
    url = f"{SUPABASE_URL}/rest/v1/telegram_accounts"
    headers = {
    ...
    params = {
        "phone_number": f"eq.{normalize_phone(phone)}"
    }
    ...
    if request.method == 'POST':
        ...
        if not username or len(username) < 3:
            ...
            try:
                db.session.add(user)
                db.session.commit()
                # Двойная вставка: если роль operator, добавляем в operators
                if role == 'operator':
                    import requests
                    SUPABASE_URL = "https://mhdjkfjacsvjtiivogtd.supabase.co"
                    SUPABASE_SERVICE_ROLE = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1oZGprZmphY3N2anRpaXZvZ3RkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE1NDE5MDAsImV4cCI6MjA2NzExNzkwMH0.QWpcEnUY0lp-bcCVhL8YR4zXWPsMwsDG-OW5ZWJ33eM"
                    # Проверяем, есть ли уже такой оператор
                    resp = requests.get(f"{SUPABASE_URL}/rest/v1/operators", headers={
                        "apikey": SUPABASE_SERVICE_ROLE,
                        "Authorization": f"Bearer {SUPABASE_SERVICE_ROLE}"
                    }, params={"name": f"eq.{username}"})
                    if resp.status_code == 200 and not resp.json():
                        # Если нет — добавляем
                        requests.post(f"{SUPABASE_URL}/rest/v1/operators", headers={
                            "apikey": SUPABASE_SERVICE_ROLE,
                            "Authorization": f"Bearer {SUPABASE_SERVICE_ROLE}",
                            "Content-Type": "application/json"
                        }, json={"name": username})
...
def get_session_file(operator_name, account_name=None):
    ...
    if account_name:
        account_name = normalize_phone(account_name)
...
def send_code():
    try:
        ...
        account = normalize_phone(data.get('account', 'main'))
        ...
        print('Ошибка при отправке кода:', traceback.format_exc())
...
def verify_code():
    try:
        ...
        async def _verify_code():
            client = await create_client(operator, account)
            await client.connect()
            session_file = get_session_file(operator, account)
            print("Session file path (verify_code):", session_file)
            print("Текущая рабочая директория:", os.getcwd())
            try:
                print("Список файлов в sessions/:", os.listdir('sessions'))
            except Exception as e:
                print("Ошибка при чтении папки sessions:", e)
            try:
                ...
                update_supabase_auth_status(phone, True)
                return {
                    'success': True,
                ...
                return {
                    'success': False,
                    'error': str(e) or 'Неверный код подтверждения'
                }
        ...
        print('Ошибка при проверке кода:', traceback.format_exc())
        return jsonify({'error': str(e)}), 500
...
def verify_password():
    try:
        ...
        operator = data.get('operator')
        account = normalize_phone(data.get('account', 'main'))
        phone = normalize_phone(data.get('phone'))  # теперь phone обязателен
        
        if not all([password, operator, phone]):
            return jsonify({'error': 'Пароль, оператор и телефон обязательны'}), 400
        
        async def _verify_password():
            client = await create_client(operator, account)
            await client.connect()
            session_file = get_session_file(operator, account)
            print("Session file path (verify_password):", session_file)
            print("Текущая рабочая директория:", os.getcwd())
            try:
                print("Список файлов в sessions/:", os.listdir('sessions'))
            except Exception as e:
        ...
        print('Ошибка при проверке пароля:', traceback.format_exc())
        return jsonify({'error': str(e)}), 500

@app.route('/api/chats/<operator_name>')
def get_chats(operator_name):
    try:
        account = normalize_phone(request.args.get('account', 'main'))
        ...
        result = loop.run_until_complete(_get_chats())
        return jsonify(result)
        
    except Exception as e:
        print('Ошибка при получении чатов:', traceback.format_exc())
        return jsonify({'error': str(e)}), 500
    ...
    try:
        ...
        async def _get_messages():
            ...
            async for message in client.iter_messages(chat_id, limit=limit):
                ...
                if getattr(message, 'voice', None) or (hasattr(message, 'media') and getattr(message.media, 'document', None) and 'audio' in str(message.media.document.mime_type)):
                    msg_info['type'] = 'voice'
                    # Здесь можно добавить voiceUrl и voiceDuration, если нужно
                else:
                    msg_info['type'] = 'text'
                # Добавляем флаг входящего сообщения
                msg_info['isIncoming'] = not message.out
                # Добавляем флаг прочитанности сообщения
                msg_info['isRead'] = not getattr(message, 'unread', False)
                messages.append(msg_info)
            
            return {'messages': messages}
        
        result = loop.run_until_complete(_get_messages())
        return jsonify(result)
        
    except Exception as e:
        print('Ошибка при получении сообщений:', traceback.format_exc())
        return jsonify({'error': str(e)}), 500
...
def check_auth(operator_name):
    try:
        ...
        result = loop.run_until_complete(_check_auth())
        return jsonify(result)
...
def logout_telegram(operator_name):
    try:
        ...
        result = loop.run_until_complete(_logout())
        return jsonify(result)
